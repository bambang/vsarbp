...
Bounds for large images. Those bounds have been extracted from the
DARPA PERFECT suite ( http://hpc.pnl.gov/PERFECT/ )
*)

Require Import ZArith RAux.
Require Flocq.Core.Fcore_Raux.
Definition abs_data_border: R := (4683475/2147483648)%R .
Definition abs_data_min: R := (43/4611686018427387904)%R .
Definition abs_data_max: R := (526709/524288)%R .
Definition abs_data_dist: R := (1450313/8388608)%R .

Definition platpos_x_min: R := (7227757/1024)%R .
Definition platpos_x_max: R := (14481547/2048)%R. 

Definition platpos_y_min: R := (0)%R .
Definition platpos_y_max: R := (13887169/32768)%R. 
...
Definition N_PULSES := (2048)%nat .
Definition N_RANGE := (2048)%Z .
Definition N_RANGE_UPSAMPLED := (16384)%Z .
Definition BP_NPIX_Y := (2048)%nat .
Definition BP_NPIX_X := (2048)%nat .
Definition ku := Eval compute in (7368997658362958 * / Fcore_Raux.Z2R (2 ^ 45))%R. 

Definition z0_low := 0%R. (* (-70)%R. *)
Definition z0_high := 0%R. (* 70%R. *)

Definition R0 := Eval compute in (5356820650524672 * / Fcore_Raux.Z2R (2 ^ 39))%R.
